    ...
    def analyze_action_verbs(self, bullet_points):
        ...
        if missing_verbs:
            ...
            if missing_ratio > 0.3:
                recommendations.append(f"Many bullet points ({int(missing_ratio*100)}%) don't start with action verbs. Start each bullet with a strong action verb.")
                if missing_verbs:
        ...
        if verb_scores:
            ...
            if avg_verb_score < 0.5:
                ...
                if weak_bullets:
                    ...
                    recommendations.append(f"Example: Replace \"{verb}\" in \"{bullet}\" with stronger alternatives like {', '.join(better_verbs)}.")
            ...
            if len(used_verbs) > 5 and len(unique_verbs) / len(used_verbs) < 0.6:
                recommendations.append("Low diversity of action verbs. Vary your word choice to highlight different skills.")
