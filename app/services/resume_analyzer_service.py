...
class PracticalResumeAnalyzer:
    def __init__(self, action_verbs_path=ACTION_VERBS_FILE, industry_skills_path=INDUSTRY_SKILLS_FILE):
        ...
        self.action_verbs_data = self._load_action_verbs_data(action_verbs_path) # Load raw data
    ...
    def _load_grammar_tool(self):
         # (Same as before - loads language_tool)
        try:
            return language_tool_python.LanguageTool(GRAMMAR_CHECK_LANG)
        except Exception as e:
            print(f"ERROR: Could not initialize LanguageTool for '{GRAMMAR_CHECK_LANG}'. Grammar check disabled. Error: {e}")
            return None
